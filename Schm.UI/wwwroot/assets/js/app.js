...
(function ($) {
    ...
    $('.toggle-menu').click(function () {
        $(".navigation-menu-body").toggle();
        $(".main-content").toggleClass('m-l-50');
        $(".navigation").toggleClass('width-80');
        $(".navigation li:not(.toggle-menu)").toggle();
        selectActiveMenu();
    });

    function selectActiveMenu() {
        var menueItem = $('a[href^="' + window.location.pathname + '"]');
        if (window.location.pathname == "/Home/Index" || window.location.pathname == "/") {
            $(".home").addClass('active');
            $("#navigationDashboards").addClass('navigation-active');
        }
        else if (!$(menueItem).hasClass('go-to-page')) {
            $('li').removeClass('active');
            $('.navigation .navigation-menu-body ul.navigation-active').removeClass('navigation-active');
            let parentUl = $(menueItem).closest('ul').attr('Id');
            $('a[href^="#' + parentUl + '"]').closest('li').addClass('active');
            $(menueItem).addClass('active');
            $(menueItem).closest('ul').addClass('navigation-active');
        }
    }
